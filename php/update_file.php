...
<?php
	...
	if($state=="New York"){
		$location = "SELECT `longitude`,`latitude`,`username`,`tweet` FROM `tweeps`";
		...
		$time = "SELECT t.tweeted_at,COUNT(t.tweeted_at) from tweeps t,hashtables h where t.unique_id=h.unique_id_tweet and h.hash_tags='$hash' group by round(UNIX_TIMESTAMP(t.tweeted_at) / 300)";
		$result3 = mysql_query($time);
		while($row2 = mysql_fetch_array($result3)){
			array_push($datetime,$row2[0]);
			array_push($count,$row2[1]);
		}
	}
	
	else if($state=="Illinois"){
		$location = "SELECT `longitude`,`latitude`,`username`,`tweet` FROM `illinois_tweep`";
		mysql_query('SET CHARACTER SET utf8');
		$result = mysql_query($location);
		...
		$time = "SELECT t.tweeted_at,COUNT(t.tweeted_at) from illinois_tweep t,illinois_hashtags h where t.unique_id=h.unique_id_tweet and h.hash_tags='$hash' group by round(UNIX_TIMESTAMP(t.tweeted_at) / 300)";
		$result3 = mysql_query($time);
		while($row2 = mysql_fetch_array($result3)){
			array_push($datetime,$row2[0]);
			array_push($count,$row2[1]);
		...
		$time = "SELECT t.tweeted_at,COUNT(t.tweeted_at) from nc_tweeps t,nc_hashtags h where t.unique_id=h.unique_id_tweet and h.hash_tags='$hash' group by round(UNIX_TIMESTAMP(t.tweeted_at) / 300)";
		$result3 = mysql_query($time);
		while($row2 = mysql_fetch_array($result3)){
			array_push($datetime,$row2[0]);
			array_push($count,$row2[1]);
		...
		$location = "SELECT `longitude`,`latitude`,`username`,`tweet` FROM `tweep_california`";
		mysql_query('SET CHARACTER SET utf8');
		$result = mysql_query($location);
	...
	echo json_encode(array($longitude,$latitude,$top_hash,$tweets,$name,$datetime,$count,mysql_num_rows($result2)));
?>